...
export default function Footer() {
  ...
  return (
    <footer className="relative bg-tb-dark border-t border-white/[0.06] overflow-hidden">
      {/* Thai pattern at 5% */}
      ...
      <div className="absolute top-0 left-1/2 -translate-x-1/2 w-96 h-48 bg-tb-gold/[0.04] blur-3xl pointer-events-none" />
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 py-16">
        <motion.div
          ...
          transition={reduceMotion ? { duration: 0.25 } : { duration: 0.7, ease: [0.22, 1, 0.36, 1] }}
        >
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 mb-12">
            {/* Brand column */}
            <div className="lg:col-span-2">
              <img src="/images/logo.png" alt="Thai Bistro" className="h-14 w-auto object-contain rounded mb-5" />
              <p className="text-white/55 text-sm leading-relaxed max-w-sm mb-6">
                A family-owned restaurant serving authentic gourmet Thai cuisine with a
                contemporary twist. We look forward to serving you.
              </p>
              <div className="flex items-center gap-2 -ml-3">
                <motion.a href="https://www.facebook.com/61577805220122" target="_blank" rel="noopener noreferrer"
                  whileTap={reduceMotion ? undefined : { scale: 0.9 }}
                  whileHover={reduceMotion ? undefined : { scale: 1.1 }}
                  className="flex items-center justify-center w-12 h-12 rounded-full text-white/45 hover:text-tb-gold hover:bg-white/[0.04] transition-colors tap-highlight-none" aria-label="Facebook">
                  <FacebookIcon />
                </motion.a>
                <motion.a href="https://www.instagram.com/thaibistroshoreline" target="_blank" rel="noopener noreferrer"
                  whileTap={reduceMotion ? undefined : { scale: 0.9 }}
                  whileHover={reduceMotion ? undefined : { scale: 1.1 }}
                  className="flex items-center justify-center w-12 h-12 rounded-full text-white/45 hover:text-tb-gold hover:bg-white/[0.04] transition-colors tap-highlight-none" aria-label="Instagram">
                  <InstagramIcon />
                </motion.a>
              </div>
            </div>
            ...
            {/* Quick links */}
            <div>
              <h4 className="text-white font-semibold text-sm tracking-[0.15em] uppercase mb-5">Quick Links</h4>
              <ul className="space-y-1">
                {quickLinks.map((link, i) => (
                  <motion.li
                    ...
                    viewport={{ once: true }}
                    transition={reduceMotion ? { duration: 0.2 } : { duration: 0.4, delay: 0.05 * i }}
                  >
                    <a href={link.href}
            ...
            {/* Contact */}
            <div>
              <h4 className="text-white font-semibold text-sm tracking-[0.15em] uppercase mb-5">Contact</h4>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 text-white/30 text-xs">
          <span>© 2026 Thai Bistro. All rights reserved.</span>
          <span>Shoreline · Federal Way · Mill Creek, WA</span>
        </div>
      </div>
    </footer>
  )
